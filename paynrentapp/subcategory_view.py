...
def DisplaySubCategorybyId(request):
    try:
        if request.method == 'GET':
            q = " select S.*,( select C.categoryname from paynrentapp_category C where C.id = S.category_id) as categoryname from paynrentapp_subcategory S where id={0}".format(request.GET['id'])
            print(q)
            cursor = connection.cursor()
            cursor.execute(q)
...
@xframe_options_exempt
@api_view(['GET','POST','DELETE'])
def DisplaySubCategorybyIdJSON(request):
    try:
        if request.method == 'GET':
